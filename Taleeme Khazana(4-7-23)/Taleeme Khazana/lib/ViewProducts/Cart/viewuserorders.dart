...
class userOrdersPage extends StatefulWidget {
  final String id;
  const userOrdersPage({Key? key, required this.id}) : super(key: key);

  @override
  State<userOrdersPage> createState() => _userOrdersPageState(id);
}

class _userOrdersPageState extends State<userOrdersPage> {
  String id;
  _userOrdersPageState(this.id);
  @override
      ...
      home: DefaultTabController(
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            bottom: const TabBar(
                tabs: [
                  Tab(icon: Text('Pending', style: TextStyle(color: Colors.white, fontSize: 20),)),
  ...
  Widget getUseOrders(MongoDbModel userdata, bool status){
    return SafeArea(
      child: FutureBuilder(
        future: mongoDatabase.getordersbystatusandid(status, id),
                  ...
                  Column(
                    children: [
                      ...
                      IconButton(
                          ...
                          icon: Icon(Icons.remove_red_eye_outlined),iconSize: 30,),
                    ],
                  )
                ],
              ),
